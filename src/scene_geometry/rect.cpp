...
float XYRect::PDFValue(const Vec3 &origin, const Vec3 &direction, float time) const {
  HitRecord rec;
  if (this->Hit(Ray(origin, direction, time), 0.001f, FLT_MAX, rec)) {
    float area = (x_max_ - x_min_) * (y_max_ - y_min_);
...
float XZRect::PDFValue(const Vec3 &origin, const Vec3 &direction, float time) const {
  HitRecord rec;
  if (this->Hit(Ray(origin, direction, time), 0.001f, FLT_MAX, rec)) {
...
float YZRect::PDFValue(const Vec3 &origin, const Vec3 &direction, float time) const {
  HitRecord rec;
  if (this->Hit(Ray(origin, direction, time), 0.001f, FLT_MAX, rec)) {
    float area = (y_max_ - y_min_) * (z_max_ - z_min_);
    float distance_squared = rec.t * rec.t * direction.squared_length();
    float cosine = fabs(dot(direction, rec.normal) / direction.length());
    return distance_squared / (cosine * area);
  } else
...
Vec3 XYRect::Random(const Vec3 &origin) const {
  Vec3 random_point = Vec3(x_min_ + drand48() * (x_max_ - x_min_),
                           y_min_ + drand48() * (y_max_ - y_min_), fixed_val_);
...
Vec3 YZRect::Random(const Vec3 &origin) const {
  Vec3 random_point = Vec3(fixed_val_, y_min_ + drand48() * (y_max_ - y_min_),
                           z_min_ + drand48() * (z_max_ - z_min_));
  return random_point - origin;
}
